...
void EnvironmentPanel::createUi()
{
    ...
    m_triPlanarWorldRotation = CreateSliderControl(0.0, 360.0, 1.0, 1);
    ...
    sourceForm->addRow(tr("Tri-Planar Scene Rotation (deg)"), m_triPlanarWorldRotation);
    ...
    connect(m_triPlanarWorldRotation->spinBox(), qOverload<double>(&QDoubleSpinBox::valueChanged), this, [this](double) {
        applyLightingSettings(false, true);
    });
...
void EnvironmentPanel::syncFromRenderer()
{
    ...
    m_triPlanarWorldRotation->setValue(g_cameraData.triPlanarWorldRotationDegrees);
...
void EnvironmentPanel::applyLightingSettings(bool updateSkyModel, bool updateCameraBuffer)
{
    ...
    g_cameraData.triPlanarWorldRotationDegrees =
        static_cast<float>(m_triPlanarWorldRotation->value());
