...
public:
    ...
    void Init(ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE type, UINT numDescriptors, UINT frameCount) {
        m_device = device;
        m_type = type;
    ...
    void ResetFrame(UINT frameIndex) {
        if (frameIndex < m_nextOffset.size()) m_nextOffset[frameIndex] = 0;
    }
...
struct FrameResource {
    ...
    std::vector<ComPtr<ID3D12Resource>> transientResources; // keep transient upload buffers alive per-frame
};
