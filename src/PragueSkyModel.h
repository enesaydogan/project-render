...
public:
    ...
    /// direction.
    ///
    /// Expects the Parameters structure, wavelength (full dataset supports wavelengths from 280 nm to 2480 nm)
    /// and the distance (any positive number, use std::numeric_limits<double>::max() for infinity).
    ///
    /// Throws NotInitializedException if called without initializing the model first.
