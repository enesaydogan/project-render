...
public:
    ...
    void SetSkyVisibility(float km) { if(m_visibility != km) { m_visibility = km; m_skyDirty = true; } }
    float GetSkyVisibility() const { return m_visibility; }

    void SetSkyAlbedo(float v) { if(m_albedo != v) { m_albedo = v; m_skyDirty = true; } }
    float GetSkyAlbedo() const { return m_albedo; }

    void SetSolarAltitude(float v) { if(m_solarElevation != v) { m_solarElevation = v; m_skyDirty = true; } }
    float GetSolarAltitude() const { return m_solarElevation; }

    void SetSolarAzimuth(float v) { if(m_solarAzimuth != v) { m_solarAzimuth = v; m_skyDirty = true; } }
    float GetSolarAzimuth() const { return m_solarAzimuth; }

    void SetObserverAltitude(float meters) { if(m_altitude != meters) { m_altitude = meters; m_skyDirty = true; } }
    float GetObserverAltitude() const { return m_altitude; }

    void SetSkyIntensity(float v) { if(m_skyIntensity != v) { m_skyIntensity = v; m_skyDirty = true; } }
