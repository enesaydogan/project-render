...
static ImTextureID GetImGuiTexIDForTextureIndex(int textureIndex) {
    if (textureIndex < 0) return (ImTextureID)0;
    if (!g_device) return (ImTextureID)0;
    if (g_texturesGpuStart.ptr == 0) return (ImTextureID)0;
    if ((UINT)textureIndex >= g_textureDescriptorCount) return (ImTextureID)0;

    const UINT inc = g_device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    if (inc == 0) return (ImTextureID)0;
...
void Draw(HWND hwnd, bool &visible) {
    ...
    if (ImGui::Begin("Material Editor", &visible)) {
        ...
        ImGui::SameLine();
        ImGui::TextDisabled("|  Loaded: %d materials, %d textures", (int)g_loadedMaterials.size(), (int)g_loadedTextures.size());
        ImGui::Separator();
        ...
        for (size_t i = 0; i < nodes.size(); ++i) {
            if (nodes[i].selected) { selectedNodeIndex = (int)i; break; }
        }
        const Scene::Node* selectedNode = (selectedNodeIndex >= 0) ? &nodes[selectedNodeIndex] : nullptr;
        ...
        if (s_showAllMaterials || !selectedNode) {
            ...
            for (int i = 0; i < (int)g_loadedMaterials.size(); ++i) materialIndices.push_back(i);
        } else {
            ...
            for (size_t i = 0; i < selectedNode->meshIndices.size(); ++i) {
                ...
                if (matIdx < 0 || matIdx >= (int)g_loadedMaterials.size()) continue;
                bool exists = false;
                for (int existing : materialIndices) {
                    if (existing == matIdx) { exists = true; break; }
                }
                if (!exists) materialIndices.push_back(matIdx);
            }
        }
        ...
        if (s_pendingMaterialSelect != -1) {
            if (s_pendingMaterialSelect >= 0 && s_pendingMaterialSelect < (int)g_loadedMaterials.size()) {
                s_selectedMaterial = s_pendingMaterialSelect;
                // If we're scoped to the selected node and the picked material isn't in it, auto-switch to all.
                if (!s_showAllMaterials && selectedNode) {
                    bool inNode = false;
                    for (int mi : materialIndices) { if (mi == s_selectedMaterial) { inNode = true; break; } }
                    if (!inNode) s_showAllMaterials = true;
                }
        ...
        if (!s_showAllMaterials && selectedNodeIndex != s_lastSelectedNodeIndex) {
            ...
            bool ok = false;
            for (int mi : materialIndices) { if (mi == s_selectedMaterial) { ok = true; break; } }
            if (!ok) s_selectedMaterial = materialIndices.empty() ? -1 : materialIndices.front();
        }
        ...
        if (leftWidth > avail.x * 0.5f) leftWidth = avail.x * 0.5f;
        ...
        ImGui::BeginChild("##mat_list", ImVec2(leftWidth, 0), true);
        {
            ...
            if (!s_showAllMaterials && !selectedNode) {
                ...
                for (int matIdx : materialIndices) {
                    if (matIdx < 0 || matIdx >= (int)g_loadedMaterials.size()) continue;
                    if (!FilterPass(matIdx)) continue;

                    const Asset::Material& m = g_loadedMaterials[matIdx];
                    std::string label = std::string(m.name) + "  ##mat_" + std::to_string(matIdx);
        ...
        ImGui::BeginChild("##mat_inspector", ImVec2(0, 0), false);
        {
            if (s_selectedMaterial < 0 || s_selectedMaterial >= (int)g_loadedMaterials.size()) {
                ...
                // Quick actions
                if (ImGui::Button("Copy")) { s_clipboard = mat; s_hasClipboard = true; }
                ImGui::SameLine();
                if (ImGui::Button("Paste") && s_hasClipboard) { mat = s_clipboard; DxrRenderer::ResetAccumulation(); }
                ImGui::SameLine();
                if (ImGui::Button("Reset")) { ResetDefaults(true); DxrRenderer::ResetAccumulation(); }
                ImGui::SameLine();
                if (ImGui::Button("Reset (No Tex)")) { ResetDefaults(false); DxrRenderer::ResetAccumulation(); }
                ...
                if (ImGui::BeginTabBar("##mat_tabs")) {
                    if (ImGui::BeginTabItem("Surface")) {
                        if (ImGui::ColorEdit3("Base Color", mat.diffuseColor)) DxrRenderer::ResetAccumulation();
                        ...
                        if (ImGui::ColorEdit3("Reflection Tint", mat.reflectionColor)) DxrRenderer::ResetAccumulation();
                    ...
                    if (ImGui::BeginTabItem("Textures")) {
                        auto DrawTextureSlot = [&](const char* label, int& idx) {
                            ...
                            // Thumbnail preview (uses engine SRV heap; requires ImGui to be bound to same heap)
                            ...
                            if (texID != (ImTextureID)0) {
                                ...
                                if (ImGui::IsItemHovered() && idx >= 0 && idx < (int)g_loadedTextures.size()) {
                                    const auto& t = g_loadedTextures[idx];
                                    ImGui::SetTooltip("#%d (%ux%u) mips=%u", idx, t.width, t.height, t.mipLevels);
                                }
                            } else {
                            ...
                            if (ImGui::Combo("##tex", &comboIdx, cstr.data(), (int)cstr.size())) {
                                ...
                                if (newIdx >= 0 && newIdx < (int)g_loadedTextures.size() && !g_loadedTextures[newIdx].resource) newIdx = -1;
                                idx = newIdx;
                                DxrRenderer::ResetAccumulation();
                            }

                            ImGui::SameLine();
                            if (ImGui::Button("Clear")) { idx = -1; DxrRenderer::ResetAccumulation(); }
                            ImGui::SameLine();
                            if (ImGui::Button("Load...")) {
                                ...
                                if (OpenTextureFileDialog(hwnd, chosen) && !chosen.empty()) {
                                    ...
                                    if (newTex >= 0) { idx = newTex; DxrRenderer::ResetAccumulation(); }
                                }
                            }
                    ...
                    if (ImGui::BeginTabItem("Mapping")) {
                        ...
                        if (ImGui::Checkbox("Enable", &tri)) { mat.triPlanarEnabled = tri ? 1.0f : 0.0f; DxrRenderer::ResetAccumulation(); }
                    ...
                    if (ImGui::BeginTabItem("Flags")) {
                        if (ImGui::Checkbox("Double Sided", &mat.doubleSided)) DxrRenderer::ResetAccumulation();
                        const char* alphaModes[] = {"OPAQUE", "MASK", "BLEND"};
                        int mode = 0;
                        if (mat.alphaMode == "MASK") mode = 1;
                        else if (mat.alphaMode == "BLEND") mode = 2;
                    ...
                    if (ImGui::BeginTabItem("QA")) {
                        ...
                        if (!isMetal && !isGlass && (aMin < 0.02f || aMax > 0.90f)) {
                            ImGui::TextColored(ImVec4(1, 0.65f, 0, 1), "Dielectric albedo is outside typical range (avoid near-black/white).\n");
                        }
                        if (rough < 0.02f) {
                            ImGui::TextColored(ImVec4(1, 0.65f, 0, 1), "Roughness < 0.02 can cause fireflies (shader clamps to 0.02).\n");
                        }
                        if (mat.clearcoat > 0.01f && mat.clearcoatRoughness < 0.02f) {
                            ImGui::TextColored(ImVec4(1, 0.65f, 0, 1), "Clearcoat roughness very low; may sparkle.\n");
                        }
                        ImGui::EndTabItem();
                    }
