...
*      Modification: take a look at @ref DuctTransitionPreferenceTable "DuctTransitionPreferenceTable"
*/
...
public:
	...
	*/
	...
	/**
	 * @brief A row is considered valid if both diameters and the transition length contains non-zero values, description is optional.
	 * @param beginReferenceId The begin referenceId of the row. The table only contains the referenceIds that are defined in the PipeReferenceSet.
	 * @param endReferenceId The end referenceId of the row. The table only contains the referenceIds that are defined in the PipeReferenceSet.
	 ...
	 * If the returned value is 0.0, the row is not valid yet.
	 */
	Result<double> GetLengthByReferenceIds (uint32_t beginReferenceId, uint32_t endReferenceId) const;
...
public:
	...
	*/
	 ...
	 * @param beginReferenceId The begin referenceId of the new row. It must be a referenceId that is defined in the PipeReferenceSet.
	 * @param endReferenceId The end referenceId of the new row. It must be a referenceId that is defined in the PipeReferenceSet.
	 ...
	 *			- if the begin diameter and end diameter are already defined, and the calculation of the transition angle is not between [1, 89] degrees.
	 */
	Result<void> SetLengthByReferenceIds (uint32_t beginReferenceId, uint32_t endReferenceId, double length);
...
using v1::PipeTransitionPreferenceTable;
