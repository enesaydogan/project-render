#ifndef GS_ACAPI_MEP_DUCT_BRANCH_PREFERENCE_TABLE_HPP
#define GS_ACAPI_MEP_DUCT_BRANCH_PREFERENCE_TABLE_HPP
...
/**
* @brief The DuctBranchPreferenceTable contains cross section and geometry related information for Duct branches.
* @ingroup MEP
* @since Archicad 28
...
* 	   Getters @snippet Preferences.cpp DuctBranchPreferenceTable-Getters-Example
*      Modification: take a look at @ref PipeBranchPreferenceTable "PipeBranchPreferenceTable"
*/
class MEPAPI_EXPORT DuctBranchPreferenceTable final : public Object {
	DuctBranchPreferenceTable (const UniqueID&);

public:
	class Modifier;
	~DuctBranchPreferenceTable ();

	/**
	 * @brief Returns a DuctBranchPreferenceTable.
	 * @param uniqueId The UniqueID of the requested DuctBranchPreferenceTable.
	 * @return If the uniqueId belongs to a DuctBranchPreferenceTable, returns the object associated to it, otherwise Error.
	 */
	static Result<DuctBranchPreferenceTable> Get (const UniqueID& uniqueId);
	...
	*/
	...
	/**
	 * @brief A row is considered valid if both diameters, the length, the branchOffset and the connectionLength contains non-zero values, description is optional.
	 * @param mainAxisReferenceId The main axis referenceId of the row. The table only contains the referenceIds that are defined in the DuctReferenceSet.
	 * @param branchAxisReferenceId The branch axis referenceId of the row. The table only contains the referenceIds that are defined in the DuctReferenceSet.
...
class MEPAPI_EXPORT DuctBranchPreferenceTable::Modifier : public ACAPI::ModifierBase {
	DuctBranchPreferenceTable& parent;

protected:
	/**
	 * @brief Constructs a Modifier with the given DuctBranchPreferenceTable.
	 * @param parent The given DuctBranchPreferenceTable.
	 */
	Modifier (DuctBranchPreferenceTable& parent);

public:
	~Modifier ();

	friend DuctBranchPreferenceTable;
	...
	*/
	 ...
	 * @param mainAxisReferenceId The main axis referenceId of the new row. It must be a referenceId that is defined in the DuctReferenceSet.
	 * @param branchAxisReferenceId The branch axis referenceId of the new row. It must be a referenceId that is defined in the DuctReferenceSet.
	 ...
	 * @return Returns error if at least one referenceId is not in the DuctReferenceSet, or if the value is smaller than the branch axis diameter, otherwise nothing.
	 */
	 ...
	 * @details If there isn't any row with the given pair of referenceIds, but both the referenceIds are defined in the DuctReferenceSet, it will insert the row.
	 * @return Returns nothing if the operation was successful, or returns error:
	 *			- if at least one referenceId is not in the DuctReferenceSet;
	 *			- if the value is larger than the already defined main axis diameter;
	 *			- if the value is larger or equal than the already defined length;
	 *			- if branch offset is already defined and the value is larger than 2 * branchOffset;
	 *			- if branch offset and length are already defined and the value is larger than 2 * (length - branchOffset).
...
template <typename ModifierFunc>
Result<void> DuctBranchPreferenceTable::Modify (const ModifierFunc& modifierFunc, const GS::UniString& undoString)
{
...
using v1::DuctBranchPreferenceTable;

} // ACAPI::MEP

#endif // GS_ACAPI_MEP_DUCT_BRANCH_PREFERENCE_TABLE_HPP