...
Texture2D<float4> g_albedoInput : register(t5); // primary-hit albedo for demodulation round-trip
...
void CSMain(uint3 id : SV_DispatchThreadID)
{
    ...
    // Re-apply the same albedo clamp used in the demodulation step in the
    // path tracer shader so the round-trip is exact (no energy gain/loss).
    float3 demodAlbedo = max(albedo, float3(0.01f, 0.01f, 0.01f));

    // Reconstruct: stable emission + denoised irradiance × albedo + denoised specular
    float3 color = max(stable.rgb + denoisedDiffuse * demodAlbedo + denoisedSpecular, 0.0f.xxx);
